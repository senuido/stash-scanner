...
class Scrolling_Area(Frame, object):
    def __init__(self, master, width=None, anchor=N, height=None, mousewheel_speed=2, scroll_horizontally=True,
        ...
        if scroll_vertically:
            if yscrollbar is not None:
                ...
                self.yscrollbar = AutoScrollbar(self, orient=VERTICAL)
        ...
        self.innerframe.bind('<Configure>', self._on_frame_configure)
    ...
    def _on_canvas_configure(self, event):
        ...
        self.canvas.itemconfigure("inner_frame", width=width)

    def _on_frame_configure(self, event):
        # width = max(self.innerframe.winfo_reqwidth(), self.canvas.winfo_width())
        # height = max(self.innerframe.winfo_reqheight(), self.canvas.winfo_height())

        self.canvas.configure(scrollregion="0 0 %s %s" % (event.width, event.height))
...
if __name__ == '__main__':
    ...
    lst_left = Listbox(root)
    lst_top = Listbox(root)
    root.rowconfigure(1, weight=1)
    root.columnconfigure(1, weight=1)
    lst_left.grid(row=0, column=0, rowspan=2, sticky='news')
    lst_top.grid(row=0, column=1, sticky='news')

    scrolling_area = Scrolling_Area(root)
    scrolling_area.grid(row=1, column=1, sticky='news')
    ...
    def addItem():
        ...
        for i in range(5):
            Label(scrolling_area.innerframe, text='Label number: {}'.format(line_num)).grid(row=line_num)
            line_num += 1


    btn = Button(root, command=addItem, text='Add Item')
